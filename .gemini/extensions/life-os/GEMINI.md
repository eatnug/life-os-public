# Life OS Gemini Extension

Use this extension when working in this Life OS repo to retrieve context, capture or update slices, maintain entities/stories, follow view workflows, run internal briefing/search/lint, or perform Life OS system maintenance.
...
`AGENTS.md` is the canonical instruction source. `_system/life-os/config.json` defines repo paths and command mappings. `_system/life-os/view-workflows.json` defines per-view lifecycle behavior.
...
This extension does not create a deterministic runtime loop. It gives Gemini the Life OS workflow and internal commands to use while keeping the user experience natural.
